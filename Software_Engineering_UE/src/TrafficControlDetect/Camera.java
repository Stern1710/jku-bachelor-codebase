...
/**
 * This class respresents a Camera - DetectionType
 * Each street is going to have at least 1 camera
 * Each camera works like a client, sending its data to 
 * it corresponding Detection Network.
 * @author Christopher Holzweber
 *
 */
public class Camera extends DetectionDevice{
	...
	public final StreetName streetID;
	public final Street street;
	SubsystemConnector connector;
	public Camera(Street name, SubsystemConnector connector) {
	...
	void start() throws UnknownHostException, IOException, InterruptedException, ClassNotFoundException {
		...
		System.out.println("Camera Client " + myId + " started and connected to server");
		...
		Thread sendDataThread = new Thread(() -> {
			...
			try {
				while (!Thread.interrupted()) {
					...
					sb.append("\",");
					sb.append("\"DetectionType\": \"Camera\",");
					sb.append("\"ID\": \"");
					sb.append(myId);
					sb.append("\",");
					sb.append("\"available\": \"");
					...
					if(street.getStreetMode() != StreetMode.BLOCK) {
						detected = 0;
						for(Unmotorized m : connector.getUnmotorizedParticipants().values()) {
							if(m.getCurrStreet() == streetID) {
								detected++;
								break;
							}
