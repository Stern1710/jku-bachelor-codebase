...
/* ---------- NON COMMAND METHODS - Auxiliary functions */

/* Methods for generating the key pair */
...
/**
 * Parses a DEC integer value (0 .. 15) from a hex number
 */ 
static int fromhex(char c) {
...
/**
 * Lists the parameters which will be used for generating the key pair 
 */
static void list_parameter_set(int levels, const param_set_t *lm_array,
                           const param_set_t *ots_array, size_t aux_size ) {
...
/**
 * Lists how to use the commands of this program.
 * Parts with "<>" are mandatory, parts with "[]" are optional
 * or in case of sign/verify 1 or more times.
 */ 
static void usage(char *program) {
    printf("Usage:\n" );
...
/**
 * Checks if the string contains the passed prefix.
 * If the prefix is present, it returns the string
 * Otherwise returns 0 as a char*
 */
