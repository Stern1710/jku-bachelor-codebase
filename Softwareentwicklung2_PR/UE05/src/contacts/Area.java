 ...
 * {@link Constants.DISTANCE} {@link Constants.DISTANCE} ddistance to the area. 
 */
public class Area {
	...
	/**
	 * Tests if this entity is within {@link Constants.DISTANCE} distance of the area. 
	 * @param area the area 
	 * @return true if this entity is within distance of the area
	 */
	public boolean isClose(Entity e) {
		return ! isWithin(e) && 
				e.getX() >= getRight() - CONTACT_DISTANCE && e.getX() < getRight() + CONTACT_DISTANCE &&
						e.getY()  >= getBottom() - CONTACT_DISTANCE && e.getY()  < getBottom() + CONTACT_DISTANCE;
	} 
	 ...
	 * @param d the number splits in x- and y-coordinate
	 * @return the array of sub-areas
	 */
	public Area[] split(int d) {
		int n = d * d; 
		int wd = getWidth() / d; 
		int hd = getHeight() / d; 
		Area[] subAreas = new Area[n];
		for (int i = 0; i < n; i++) {
			subAreas[i] = 
					new Area(getLeft() + wd * (i % d), 
							 getTop() + hd * (i / d), 
							 wd, 
							 hd); 
		}
