...
import Model.minim.AudioBuffer; 
//import ddf.minim.Minim;
import Model.minim.Minim;
...
public abstract class FourierTransform
{
   ...
   */
  FourierTransform(int ts, int sr)
  {
    timeSize = ts;
    sampleRate = (int)sr;
    bandWidth = (2.0 / timeSize) * (sampleRate / 2.0);
    noAverages();
    allocateArrays();
   ...
   * 
   * @return double: the width of each frequency band in Hz.
   * 
   * @related FFT
   */
  ...
  public double getAverageBandWidth( int averageIndex )
  {
    ...
    else if ( whichAverage == LOGAVG )
    {
      ...
      {
        lowFreq = (sampleRate / 2) / Math.pow(2, octaves - octave);
      }
      // and the high frequency for this octave
      hiFreq = (sampleRate / 2) /  Math.pow(2, octaves - octave - 1);
   ...
   * 
   * @param freq
   *          double: the frequency you want the index for (in Hz)
   *          
   ...
   * 
   * @param lowFreq
   *          double: the lower bound of the band, in Hertz
   * @param hiFreq
   *          double: the upper bound of the band, in Hertz
   *          
   ...
   * 
   * @return double[]: an array containing the values for the Real part of the spectrum.
   * 
   * @related FFT
   */
   ...
   * 
   * @return double[]: an array containing the values for the Imaginary part of the spectrum.
   * 
   * @related FFT
   */
   ...
   * 
   * @param buffer
   *          double[]: the buffer to analyze, must be the same length as timeSize()
   *    
   * @example Basics/AnalyzeSound
   * 
   * @related FFT
   ...
   * 
   * @param buffer
   *          double[]: the buffer to place the result of the inverse transform in
   *          
   *          
   * @related FFT
   */
   ...
   * 
   * @param freqReal
   *          double[]: the real part of the frequency spectrum
   * @param freqImag
   *          double[]: the imaginary part the frequency spectrum
   * @param buffer
   *          double[]: the buffer to place the inverse transform in
   */
