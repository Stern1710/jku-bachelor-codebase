...
class Minimax():
    ...
    def minimax(self, game, node, max_player):
        ...
        if node.player == max_player:
            ...
            for suc in game.successors(node):  # Iterate over successors
                suc_val, suc_node = self.minimax(game, suc, max_player)  # Recursive descent
                if suc_val > best_value:  # Check if value is greater, then set new best values
                    best_value = suc_val
                    best_node = suc_node
            ...
            for suc in game.successors(node):  # Iterate over successors
                suc_val, suc_node = self.minimax(game, suc, max_player)  # Recursive descent
                if suc_val < best_value:    # Check if value is smaller, then set new best values
                    best_value = suc_val
                    best_node = suc_node
