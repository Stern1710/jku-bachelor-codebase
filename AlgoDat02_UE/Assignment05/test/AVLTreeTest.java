...
class DebugTree extends AVLTree {
	...
	private boolean checkSymmetricReferences(final AVLNode n) {
		if (n != null) {
			final AVLNode parent = getParent(n), left = n.left, right = n.right;
	...
	private AVLNode getParent(final AVLNode n) {
		if (n == null || n == getRoot()) return null;

		for (AVLNode curr = getRoot(); curr != null; ) {
	...
	private boolean isAVLTreeStructure(final AVLNode n) {
		if (n != null) {
			final AVLNode left = n.left, right = n.right;
	...
	private String toString(final AVLNode node) {
